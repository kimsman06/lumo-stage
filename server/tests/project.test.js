...
const { createAuthenticatedAgent } = require("./utils/auth");
const {
  createProject,
  createShare,
  deleteProject,
...
describe("Project API", () => {
  ...
  describe("POST /api/projects", () => {
    ...
    it("인증된 사용자는 프로젝트를 생성할 수 있고 Scene 데이터가 정규화된다", async () => {
      ...
      const { project, response } = await createProject(agent, {
        name: "나의 첫 프로젝트",
      ...
      expect(response.body.project.owner).toBe(user.id);
  ...
  describe("GET /api/projects", () => {
    it("사용자의 프로젝트 목록을 반환한다", async () => {
      ...
      const response = await fetchProjects(agent);
  ...
  describe("GET /api/projects/:id", () => {
    it("자신의 프로젝트는 조회할 수 있다", async () => {
      ...
      const response = await fetchProjectDetail(agent, project.id);
    ...
    it("다른 사용자의 프로젝트는 조회할 수 없다", async () => {
      const { agent } = await createAuthenticatedAgent({
        email: "detail-owner2@example.com"
      });
      ...
      const response = await fetchProjectDetail(otherAgent, project.id);
    ...
    it("유효하지 않은 ID는 400을 반환한다", async () => {
      const { agent } = await createAuthenticatedAgent({
        email: "detail-invalid@example.com"
      });

      const response = await fetchProjectDetail(agent, "not-a-valid-id");
  ...
  describe("PATCH /api/projects/:id", () => {
    it("프로젝트 정보를 수정할 수 있다", async () => {
      ...
      const updateResponse = await updateProject(agent, project.id, {
        name: "수정 후",
        description: "설명 변경",
        sceneData: {
          nodes: [{ id: 3 }],
    ...
    it("다른 사용자의 프로젝트는 수정할 수 없다", async () => {
      ...
      const response = await updateProject(otherAgent, project.id, {
        name: "해킹 시도"
      });
    ...
    it("수정할 필드가 없으면 400을 반환한다", async () => {
      ...
      const response = await updateProject(agent, project.id, {});
    ...
    it("잘못된 ID는 400을 반환한다", async () => {
      ...
      const response = await updateProject(agent, "invalid-id", {
        name: "무효"
      });
  ...
  describe("DELETE /api/projects/:id", () => {
    it("프로젝트를 삭제할 수 있다", async () => {
      ...
      const response = await deleteProject(agent, project.id);
    ...
    it("다른 사용자가 삭제를 시도하면 404를 반환한다", async () => {
      ...
      const response = await deleteProject(otherAgent, project.id);
    ...
    it("잘못된 ID는 400을 반환한다", async () => {
      ...
      const response = await deleteProject(agent, "bad-id");
  ...
  describe("프로젝트 공유 API", () => {
    it("공유 링크를 생성하고 조회 및 공개 접근이 가능하다", async () => {
      ...
      const createResponse = await createShare(agent, project.id, {
        permission: "view"
      });
      ...
      const publicResponse = await fetchPublicShare(share.token);
      ...
      const listResponse = await fetchProjects(agent);
    ...
    it("공유 설정 업데이트로 권한과 활성 상태를 제어할 수 있다", async () => {
      const { agent } = await createAuthenticatedAgent({
        email: "share-update@example.com"
      });
      const { project } = await createProject(agent);
      ...
      const updateResponse = await updateShare(agent, project.id, {
        permission: "edit",
        expiresAt,
        isActive: false
      });
      ...
      const reactivateResponse = await updateShare(agent, project.id, {
        isActive: true
      });
      ...
      const afterReactivateList = await fetchProjects(agent);
    ...
    it("링크를 재생성하면 이전 토큰이 만료된다", async () => {
      const { agent } = await createAuthenticatedAgent({
        email: "share-regenerate@example.com"
      });
      const { project } = await createProject(agent);
      ...
      const regenerateResponse = await regenerateShare(agent, project.id);

      expect(regenerateResponse.statusCode).toBe(200);
      expect(regenerateResponse.body.share.token).not.toBe(originalToken);

      const oldTokenResponse = await fetchPublicShare(originalToken);
      ...
      const newTokenResponse = await fetchPublicShare(regenerateResponse.body.share.token);
