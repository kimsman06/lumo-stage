...
function Experience() {
  ...
  const mannequinToControl =
    !selectedBoneName && mannequinRefs.current.get(selectedMannequinId);
  ...
  useEffect(() => {
    ...
    if (transformCtrl) {
      ...
      return () =>
        transformCtrl.removeEventListener("dragging-changed", callback);
    }
          ...
          <React.Fragment key={light.id}>
            ...
            {(({ id, type, ...rest }) => {
              switch (type) {
                ...
                case "spot":
                  return (
                    <spotLight {...rest} target={currentLightTargetObject} />
                  );
                case "directional":
                  return (
                    <directionalLight
            ...
            {(light.type === "directional" || light.type === "spot") && (
              <Sphere
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setSelectedLight(`${light.id}-target`);
                  selectMannequin(null);
                }}
