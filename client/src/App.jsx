...
// 2. 컨트롤 패널 UI 컴포넌트 (변경 없음)
function Controls() {
  const { lights, addLight, updateLight, updateLightPosition, selectedLight, setSelectedLight } = useStore();
...
// 3. Scene 컴포넌트 (TransformControls 최종 적용)
function Scene() {
  const { lights, selectedLight, setSelectedLight, updateLightPositionArray } = useStore();
  ...
  return (
    <Canvas shadows camera={{ position: [0, 2, 8], fov: 75 }}>
      ...
      {/* objectToControl이 있을 때만 TransformControls를 활성화 */}
      {objectToControl && (
        <TransformControls
